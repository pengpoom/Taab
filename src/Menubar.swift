...
class Menubar {
    ...
    private static var secondaryClickMonitor: Any?
    ...
    static func initialize() {
        ...
        menu.addItem(NSMenuItem.separator())
        addMenuItem(String(format: NSLocalizedString("About %@", comment: "Menubar option. %@ is Glide"), App.name), #selector(App.showAboutWindow), "", "info.circle", nil, App.self)
        menu.addItem(NSMenuItem.separator())
        addMenuItem(String(format: NSLocalizedString("Quit %@", comment: "%@ is Glide"), App.name), #selector(NSApplication.terminate(_:)), "q", nil) // "xmark.rectangle" is not necessary; macos automatically recognizes Quit
        // Let AppKit own the primary-click menu interaction. Besides handling keyboard and
        // accessibility activation correctly, this opts into the current macOS menu-bar material
        // and rounded highlight instead of the legacy square blue highlight drawn by popUpMenu(_:).
    ...
    // Keep Glide's established secondary-click shortcut (show the switcher) while primary clicks
    // are handled natively through `statusItem.menu`. A local monitor sees the status-item window
    // before AppKit opens its menu; consuming only rightMouseDown leaves all native left-click,
    // keyboard, and VoiceOver behavior untouched.
    private static func installSecondaryClickMonitor() {
        secondaryClickMonitor = NSEvent.addLocalMonitorForEvents(matching: .rightMouseDown) { event in
...
private final class MenubarMenuDelegate: NSObject, NSMenuDelegate {
    ...
    func menuWillOpen(_ menu: NSMenu) {
        ...
        GlideSidebarCoordinator.shared.refreshMenu()
    }
}
...
class PermissionCallout: StackView {
    ...
    static func message(_ dependentFeatures: PermissionCalloutResolver.DependentFeatures) -> String {
        ...
        return String(format: NSLocalizedString("Taab is running without Screen Recording permissions. %@ won’t show.", comment: "Menubar callout. %@ is one or more feature names, e.g. Thumbnails"), subject)
    }
}
