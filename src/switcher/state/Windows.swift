...
class Windows {
    ...
    /// moved on (cmd-N burst, then open Glide) — where the `isActive` guard in `bumpFocusOrder` drops it and
    ...
    private static func refreshIfWindowShouldBeShownToTheUser(_ window: Window, _ f: WindowFilters) {
        ...
        window.shouldShowTheUser = WindowFilterResolver.shouldShow(
            ...
            onlyPreferredScreen: f.screensToShow == .showingGlide,
    ...
    static func logTileDump(_ context: String) {
        ...
        let tiles = list.enumerated().map { (i, w) -> String in
            ...
            // Glide's "one tab, one window" model comes from CGS wids — so the element may well outlive the
    ...
    /// windows Glide was not running to watch being focused. The result is applied by the reducer's
