...
final class TestScenarioSimulator {
    ...
    /// `preexisting` is the COLD-START condition: windows that already existed when Glide launched, seeded
    ...
    init(ordering: Ordering = .inOrder, handoverOrder: HandoverOrder = .asEmitted,
        ...
        // A pre-existing fullscreen window's Space is part of the topology Glide reads at launch, even
    ...
    /// window. A window Glide has never tracked doesn't count (it can't show what it hasn't discovered), nor
