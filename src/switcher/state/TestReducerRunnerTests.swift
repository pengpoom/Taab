...
final class TestReducerRunnerTests: XCTestCase {
    ...
    /// wids joining and leaving the fullscreen Space, so when the tracked active's own 1326 lands, Glide
    ...
    /// fullscreen switch to a REUSED background wid carries no focus signal Glide can see — no 808, no
    ...
    /// showing the pre-Cmd+` order (the second window is focused but Glide "didn't notice").
    ...
    /// #5875: alt-tab into Preview, then Cmd+` a beat later. Glide raises exactly ONE window, so its
    ...
    func testInAppRaiseBumpsRightAfterAnGlideInitiatedActivation() {
    ...
    func testSecondGlideIntoTheSameAppIsNotMistakenForTheActivationsRaiseTail() {
    ...
    /// Glide logged every event. The whole burst lands in ONE millisecond, and each window reports TWICE:
    ///
    ...
    /// With no focus history at all, screen stacking IS the order: Glide launched into a desktop it did not
