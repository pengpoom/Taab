...
enum ReducerInput: Equatable {
    ...
    /// NSWorkspace didActivateApplication (no WS equivalent). `altTabTargetWid` = a fresh Glide-initiated
    ...
    /// (`Windows.sortByLevel`). The order to fall back on for windows Glide has never seen focused.
