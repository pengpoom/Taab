...
/// action emits the WindowServer/AX event sequence Glide would observe, as `TestReducerRunner.Step`s. It
...
struct TestInteractionModel {
    ...
    /// A window that already existed when Glide started. See `seed`.
    ...
    /// Seed a world that Glide NEVER SAW BEING BUILT — no create, no Space join, no focus, nothing. This is
    /// the cold start: Glide launches (or is updated, or is installed mid-session) into a desktop that
    ...
    /// order-in is the whole of what Glide hears.
    ...
    private mutating func show() -> ActionEvents {
        ...
        e.ordered = [.sortByMru]
        // Which actives Glide knows about is snapshotted HERE, before any of this show's discoveries land:
        ...
        // brute-force AX adoption of INACTIVE tabs Glide never tracked (you switched away from a tab before
        ...
        // a world Glide does not live in. See `tabsReadable` and `AXUIElement.tabGroupInfo`.
        ...
        // The map is built from Glide's TRACKED windows, so an UNTRACKED minted active contributes nothing —
