...
class LicenseManager {
    ...
    private var localBuildEnabled = false
    ...
    /// Glide is a local, self-contained build. It should never depend on an account, keychain
    /// item, trial clock, or license server to expose window-management features.
    func initializeLocalBuild() {
        localBuildEnabled = true
        state = .pro
    }
    ...
    func computeState() -> LicenseState {
        if localBuildEnabled { return .pro }
