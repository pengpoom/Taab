    ...
    private static let cgEventHandler: CGEventTapCallBack = { _, type, cgEvent, _ in
        ...
        case .flagsChanged:
            ...
            DispatchQueue.main.async {
                ...
                // it was pressed down before Glide was triggered, so we should let the up event through
                handleKeyboardEvent(nil, nil, nil, modifiers, false)
            }
            return Unmanaged.passUnretained(cgEvent)
        case .keyDown:
            // Issue #5585. Esc only — absorb when Glide is using it and a shortcut binds it. cghid is
