...
/// The WindowServer event tap: Glide's source of truth for window lifecycle, focus, geometry and Space
...
class WindowServerEvents {
    ...
    /// The window Glide itself just focused (switcher selection / CLI --focus), consumed by the next
    ...
    private static var altTabInitiatedFocus: ActivationFocusResolver.GlideFocusIntent?
    ...
    static func noteGlideInitiatedFocus(_ wid: CGWindowID, _ pid: pid_t) {
    ...
    static func observe() {
        ...
        center.addObserver(forName: NSWorkspace.didActivateApplicationNotification, object: nil, queue: .main) { note in
            if let app = runningApp(note) {
                ...
                // an app already frontmost when Glide (re)launches never got a bump at all. This app-activation
                ...
                // Glide-initiated target bumps directly, when the AX backstop runs — live in
                // `WindowEventReducer.appActivated` (+ `ActivationFocusResolver`); the timing/consume of the
                // one-shot Glide-initiated intent stays here (it's this tap's own bookkeeping).
