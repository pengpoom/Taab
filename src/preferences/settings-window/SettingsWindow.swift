...
class SettingsWindow: NSWindow {
    ...
    private let quitButton = NSButton(title: String(format: NSLocalizedString("Quit %@", comment: "%@ is Glide"), App.name), target: nil, action: #selector(NSApplication.terminate(_:)))
    ...
    private func setupSidebar() {
        ...
        if GlideProductDefaults.showsLicenseUI {
    ...
    private func setupSidebarTable(_ parent: NSView) {
        ...
        NSLayoutConstraint.activate([
            ...
            sidebarScrollView.bottomAnchor.constraint(
                equalTo: GlideProductDefaults.showsLicenseUI ? upgradeButton.topAnchor : quitButton.topAnchor,
                constant: -10),
        ])
    }
