...
final class NativeHotkeyResolverTests: XCTestCase {
    ...
    /// Glide's default trigger (⌥⇥ / hold ⌥) doesn't overlap any native command-tab hotkey, so
    /// every native hotkey stays enabled — Glide coexists with the system's native switcher.
