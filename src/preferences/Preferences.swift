...
class Preferences {
    static var defaultValues: [String: Any] = {
        ...
        (0..<maxShortcutCount).forEach { index in
            // Glide replaces the native macOS app switcher. Keep the familiar Windows-style
            // Command-Tab chord as the out-of-box experience; the shortcut remains editable.
            values[indexToName("holdShortcut", index)] = defaultShortcut(GlideProductDefaults.switcherHoldShortcut)
            values[indexToName("nextWindowShortcut", index)] = defaultShortcut(index == 0 ? GlideProductDefaults.switcherNextWindowShortcut : (index == 1 ? keyAboveTabDependingOnInputSource() : ""))
        }
        (0...maxShortcutCount).forEach { index in
...
class CachedUserDefaults {
    ...
    /// these may be nil since we they don't have default values from Glide
