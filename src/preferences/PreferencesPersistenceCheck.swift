...
/// Launch-time guard that warns when macOS isn't persisting Glide's preferences to disk (issue #5790).
///
/// It gathers, off the main thread, the filesystem state of every `UserDefaults` suite Glide writes, asks
...
enum PreferencesPersistenceCheck {
    /// Every suite Glide persists to. `UserDefaults.standard` is keyed by the bundle id; the others are
    ...
    private static func showDialog(symlinkedPaths: [String], unwritablePaths: [String]) {
        ...
        alert.messageText = NSLocalizedString("Your settings reset every time Taab restarts.", comment: "")
    ...
    private static func dialogContent(symlinkedPaths: [String], unwritablePaths: [String]) -> (message: String, paths: [String]) {
        if !symlinkedPaths.isEmpty {
            return (NSLocalizedString("Taab preferences file is a symlink. macOS doesn’t support symlink for preferences. Please use a valid file instead of a symlink here:", comment: ""), symlinkedPaths)
        }
        return (NSLocalizedString("Taab preferences file isn’t writable by your account. Please fix ownership and permissions of the file here:", comment: ""), unwritablePaths)
    }
