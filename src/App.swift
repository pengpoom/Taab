...
class App: AppCenterApplication {
    ...
    static func restart() {
        ...
        // we use Bundle.main.bundlePath in case of multiple Glide versions on the machine
        printStackTrace()
    ...
    static func checkIfShortcutsShouldBeDisabled(_ activeWindow: Window?, _ activeApp: Application?) {
        ...
        // the toggle flapped and a re-check re-enabled the shortcut mid-fullscreen — Glide then grabbed Cmd-Tab
    ...
    static func continueAppLaunchAfterPermissionsAreGranted() {
        ...
        // Glide's shortcut registered after an auto-update relaunch until the user switches away and back.
        ...
        Logger.info { "Finished launching Taab" }
    }
}
...
extension App: NSApplicationDelegate {
    func applicationDidFinishLaunching(_ aNotification: Notification) {
        ...
        Logger.initialize()
        Logger.info { "Launching Taab \(App.version)" }
        ...
        #if DEBUG
        if CommandLine.arguments.contains("--visualize-constraints") {
        ...
        LicenseManager.shared.initializeLocalBuild()
