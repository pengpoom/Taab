...
struct ActivationEntry: Equatable {
    ...
    /// WRONG when there is no tail at all: an Glide-initiated activation raises exactly one window, so it
...
enum ActivationFocusResolver {
    ...
    ///
    /// **Except when the target was MINIMIZED**, which is the one focus where Glide does more than raise a
    ...
    /// The one-shot record Glide keeps when IT focused a window: the activation that follows is OURS, so the
    /// target is known (no need to divine it from a racy 808 / AX read) and there is no raise tail to mute.
    struct GlideFocusIntent: Equatable {
        var wid: CGWindowID
        var pid: pid_t
        var at: TimeInterval
    }
    ...
    /// Whether Glide's focus of `pid`'s window is worth recording — i.e. whether an activation is actually
    ...
    static func altTabIntentToRecord(wid: CGWindowID, pid: pid_t, frontmostPid: pid_t?,
                                     at: TimeInterval) -> GlideFocusIntent? {
        guard frontmostPid != pid else { return nil }
        return GlideFocusIntent(wid: wid, pid: pid, at: at)
    }
    ...
    static func altTabIntentApplies(_ intent: GlideFocusIntent?, activatedPid: pid_t, now: TimeInterval) -> Bool {
