...
* Run `scripts/codesign/setup_local.sh` once. It creates an isolated `GlideDevelopment` keychain and a 10-year local signing identity, so macOS privacy permissions survive rebuilds without depending on the login-keychain password.
* Run `scripts/build_local.sh` to unlock that dedicated keychain and build the Debug app. You can also open `Glide.xcodeproj` in Xcode after running the setup script.
